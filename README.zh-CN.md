...
| `wipe` | 尽力安全擦除（3 遍覆写后删除；SSD/CoW 文件系统无法绝对保证物理抹除） | 选择范围：1) 仅需保存的备份文件 2) 全部 → 文件清单 → 确认短语 `PERMANENT DELETE` |

### 助记词（可选，默认关闭）

- **仅支持 24 词**：12 词仅 128 位 < 256 位私钥（容量约束）。
- **标准 BIP-39/44 语义**：钱包根 = 32 字节熵（2-of-3 分片保护）；24 词 = 熵的标准 BIP-39 编码，经 `m/44'/60'/0'/0/0` 派生账户私钥——**可导入 MetaMask / Ledger / Trust Wallet 恢复同一地址**。可随时从任意 2 片导出。
...
两种范围均先随机覆写 3 遍再删除（尽力安全擦除——SSD、APFS/CoW 文件系统与快照环境
无法由应用层保证物理抹除；删除失败会如实报错，不会伪装成功），且需输入确认短语
`PERMANENT DELETE`。
...
| `signed_request_sign` | `signed_request`、`unlock_token` | 双闸门（平台背书 + 用户确认）；签名 = 域分离请求上下文（wallet_address/platform_address/action/intent_hash/nonce/expires_at/user_id） |
...
mcp-server                  薄壳（凭证不进 LLM；独立无密钥进程为路线图），接线双闸门
```
