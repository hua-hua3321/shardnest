...
3. **确认备份分布**——无邮箱时本地存 2 片，会显示显著警告。**注意：转移文件≠分离**——`recovery-codes.txt` 里两片在一起，整个文件转移到新位置后两片仍同处，2-of-3 形同虚设。请运行 `shardnest split-recovery <目录>` 把两片拆成两个独立文件，将其中一份离线保存（纸/密码管理器/另一台设备）后删除本机对应副本
