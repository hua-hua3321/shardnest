...
async function main() {
  ...
  switch (cmd) {
    ...
    case 'init': {
      ...
      console.log(t('  ℹ️  说明：12 词无法承载 32 字节私钥（容量不足），故仅支持 24 词；分片恢复码机制不受影响，二者并存', "  ℹ️ Note: 12 words cannot hold a 32-byte key (capacity); only 24 words supported; share system unaffected, both coexist"))
