...
async function main() {
  ...
  switch (cmd) {
    ...
    case 'init': {
      ...
      console.log(t('  ✅ 生成：单凭 24 词即可恢复钱包（最简恢复路径）。注意：这是 shardnest 专用格式（私钥直接编码，非标准 BIP-39 语义）——不兼容 MetaMask 等主流钱包，请勿在其他钱包中导入', "  ✅ Yes: recover with just the 24 words (simplest path). Note: shardnest-proprietary format (private key encoding, NOT standard BIP-39 semantics) — do NOT import into MetaMask etc."))
      ...
      console.log(t('  ℹ️  说明：本助记词为 shardnest 专用编码（私钥直接作熵，不走标准 BIP-39 seed→BIP-32 派生）——导入 MetaMask 等钱包会得到不同地址。12 词无法承载 32 字节私钥；分片恢复码机制不受影响，二者并存', "  ℹ️ Note: shardnest-proprietary encoding (private key as entropy, NOT standard BIP-39 seed→BIP-32 derivation) — importing these words into MetaMask etc. yields a DIFFERENT address. 12 words cannot hold a 32-byte key; share system unaffected."))
    ...
    case 'restore-mnemonic': {
      ...
      const expected = await prompt(t('期望地址（可选，强烈建议输入以校验恢复正确性，回车跳过）: ', "Expected address (optional, strongly recommended to verify recovery; Enter to skip): "))
